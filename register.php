...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($password) || empty($confirm)) {
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            $prefs_esc  = mysqli_real_escape_string($conn, $preferences);
            
            // Handle optional Date of Birth
            $dob_val = !empty($dob) ? "'" . mysqli_real_escape_string($conn, $dob) . "'" : "NULL";

            /* --- INSERT NEW USER --- */
            // Updated to include gender, dob, and preferences based on database structure
            $sql = "INSERT INTO users (name, email, phone, password, role, status, gender, dob, preferences)
                    VALUES ('$name_esc', '$email_escaped', '$phone_esc', '$hashed', 'client', 'active', '$gender_esc', $dob_val, '$prefs_esc')";
...
<head>
    ...
    <style>
        ...
        body { font-family: var(--font-body); background: var(--cream); min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 30px 16px; }
        .register-wrapper { width: 100%; max-width: 500px; }
        .brand-header { text-align: center; margin-bottom: 32px; }
        .brand-header .brand-name { font-family: var(--font-display); font-size: 36px; color: var(--charcoal); letter-spacing: 0.04em; }
        .gold-line { width: 40px; height: 2px; background: var(--gold); margin: 10px auto; border-radius: 2px; }
        .register-card { background: white; border-radius: 18px; padding: 36px 38px; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 4px 24px rgba(0,0,0,0.06); }
        .card-title { font-family: var(--font-display); font-size: 24px; color: var(--charcoal); margin-bottom: 6px; }
        .card-subtitle { font-size: 13px; color: #aaa; margin-bottom: 26px; }
        .form-label-sm { font-size: 11px; font-weight: 500; color: #999; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 6px; display: block; }
        .form-input { width: 100%; padding: 10px 14px; border: 1px solid rgba(0,0,0,0.1); border-radius: 9px; font-size: 13px; transition: border-color 0.15s; }
        .form-input:focus { border-color: var(--gold); outline: none; box-shadow: 0 0 0 3px rgba(201,168,76,0.08); }
        .input-group-custom { position: relative; }
        .input-group-custom .form-input { padding-left: 38px; }
        .input-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #ccc; font-size: 15px; }
        .toggle-password { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: #ccc; background: none; border: none; cursor: pointer; }
        .btn-gold { width: 100%; padding: 12px; background: var(--gold); border: none; border-radius: 9px; color: white; font-weight: 500; transition: 0.15s; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .bottom-link { text-align: center; margin-top: 22px; font-size: 13px; color: #aaa; }
...
<div class="register-wrapper">
    ...
    <div class="register-card">
        ...
        <form method="POST" action="">
            ...
            <div class="form-gap">
                <label class="form-label-sm">Service Preferences</label>
                <textarea name="preferences" class="form-input" placeholder="e.g. Skin allergies, preferred hair length, etc." rows="2"><?php echo htmlspecialchars($preferences ?? ''); ?></textarea>
            </div>
